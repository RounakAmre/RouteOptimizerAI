...
// Helpers to normalize & clamp AI answer
...
function normalizeRules(raw){
  ...
  let urgent_weight      = toNum(pick(raw,'urgent_weight','urgent',1.0),1.0);
  let lateness_weight    = toNum(pick(raw,'lateness_weight','lateness',1.0),1.0);
  let wait_weight        = toNum(pick(raw,'wait_weight','wait',0.1),0.1);
  let avoid_road_penalty = toNum(pick(raw,'avoid_road_penalty','avoidRoadPenalty',1200),1200);

  // Clamp sensible minimums so AI always has effect
